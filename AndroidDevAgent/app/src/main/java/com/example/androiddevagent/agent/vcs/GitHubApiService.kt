...
class GitHubApiService @Inject constructor(
    ...
    suspend fun listDirectory(path: String = "", branch: String = ""): Result<List<GitHubDirEntry>> {
        return try {
            val repo = currentRepo ?: return Result.failure(Exception("未连接仓库"))
            val b = branch.ifEmpty { repo.branch }
    ...
    suspend fun listBranches(): Result<List<GitHubBranchInfo>> {
        return try {
            val repo = currentRepo ?: return Result.failure(Exception("未连接仓库"))
    ...
    suspend fun createBranch(branchName: String, fromBranch: String = ""): Result<String> {
        return try {
            val repo = currentRepo ?: return Result.failure(Exception("未连接仓库"))
    ...
    suspend fun getRepoInfo(): Result<String> {
        return try {
            val repo = currentRepo ?: return Result.failure(Exception("未连接仓库"))
    ...
    suspend fun getRecentCommits(count: Int = 10): Result<String> {
        return try {
            val repo = currentRepo ?: return Result.failure(Exception("未连接仓库"))
    ...
    suspend fun searchCode(query: String): Result<String> {
        return try {
            val repo = currentRepo ?: return Result.failure(Exception("未连接仓库"))
